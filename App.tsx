...
const RESOURCE_LINKS: ResourceLink[] = [
  { 
    ...
    color: '#FFF9E1' // Lighter creamy yellow
  },
  { 
    label: '关于', 
    type: 'page', 
...
export default function App() {
  ...
  return (
    <div className="min-h-[100dvh] bg-transparent lg:bg-[#e8e4da] font-sans text-black relative selection:bg-[#A2D2FF] selection:text-black overflow-x-hidden flex flex-col">
      ...
      {/* =========================================================================
          2.0 MOBILE & TABLET SIDEBAR NAVIGATION (lg:hidden)
         ========================================================================= */}
        ...
        <div className="flex flex-col gap-1 pointer-events-auto items-start">
            ...
            {RESOURCE_LINKS.map((link) => {
                ...
                <button key={link.label} onClick={() => handleResourceClick(link)} className={`relative h-12 w-7 rounded-r-md border-y border-r border-black/10 shadow-sm flex items-center justify-center transition-transform duration-200 -translate-x-1 hover:translate-x-0 bg-white ${isCurrentDB ? 'translate-x-0 w-9 font-bold shadow-md' : ''}`} style={{ backgroundColor: link.color }}>
                    <span className={`block [writing-mode:vertical-rl] font-mono text-[9px] font-bold text-black/60 tracking-tight ${isCurrentDB ? 'text-black opacity-100' : ''}`}>{link.label}</span>
                </button>
             )})}
        </div>
      </nav>
      ...
      {/* =========================================================================
          2.1 DESKTOP NAVIGATION (lg:block)
         ========================================================================= */}
      ...
      <main className="flex-grow pl-12 lg:px-8 lg:pb-12 z-20 pt-0">
        <div className="max-w-7xl mx-auto min-h-[100dvh] lg:min-h-[85vh] bg-white lg:bg-[#FDFBF7] lg:rounded-b-lg lg:rounded-tr-lg rounded-none shadow-none lg:shadow-[0_4px_20px_rgba(0,0,0,0.08),0_1px_0_rgba(0,0,0,0.1)] relative border-t-0 lg:border-t border-black/5">
