...
export const mapNotionResultToSchemes = (notionData: any): Scheme[] => {
  ...
  return notionData.results.map((page: any, index: number) => {
    ...
    const getUrl = (keys: string[]) => {
        ...
        else if (prop?.type === 'rich_text') url = prop.rich_text?.[0]?.plain_text || '';
        if (!url) return '';
        url = url.trim();
        if (!/^(https?:\/\/|\/|#|mailto:)/i.test(url)) return `https://${url}`;
        return url;
    }
    ...
    // IMPROVED: Speed up by avoiding body block fetching
    const getImage = (keys: string[]) => {
       // 1. Check Native Page Cover (Best for performance)
       if (page.cover) {
           if (page.cover.type === 'external') return page.cover.external.url;
           if (page.cover.type === 'file') return page.cover.file.url;
       }
       // 2. Check Database Properties (Image/Preview field)
       const prop = getProp(keys);
       if (prop?.type === 'files' && prop.files.length > 0) {
           const file = prop.files[0];
           return file.type === 'file' ? file.file.url : file.external.url;
       }
       // 3. Fallback to API-injected content image (if still present)
